...
void fprintSeq(FILE *fp, Gene *seq, int N) {
    ...
    for(int i=0; i<N; i++)
        fprintf(fp, "%c", seqPos(i) == 1 ? '+' : '-');
    fprintf(fp, " %d %.2f", autocorrelation(seq, N), seqMeritFactor(seq, N));
}

/** Borko */
void fprintSeq2(FILE *fp, Gene *seq, int N) {
    int NDiv2 = N / 2;
    int NPlus1Div2 = (N+1) / 2;
    int NMinus1 = N-1;
    ...
    for(int i=0; i<(N+1)/2; i++)
        fprintf(fp, "%c", seqPos(i) == 1 ? '0' : '1');
    fprintf(fp, ":%d", autocorrelation(seq, N));
}

/** Borko */
void fprintSeq3(FILE *fp, Gene *seq, int N) {
    int NDiv2 = N / 2;
    int NPlus1Div2 = (N+1) / 2;
    int NMinus1 = N-1;
...
void fprintSeq4(FILE *fp, Gene *seq, int N) {
    int NDiv2 = N / 2;
    int NPlus1Div2 = (N+1) / 2;
    int NMinus1 = N-1;
...
void fprintSeq5(FILE *fp, Gene *seq, int N) {
    int NDiv2 = N / 2;
    int NPlus1Div2 = (N+1) / 2;
    int NMinus1 = N-1;
